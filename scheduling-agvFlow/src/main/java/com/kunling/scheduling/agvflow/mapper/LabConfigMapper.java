...
public interface LabConfigMapper extends BaseMapper<LabConfigEntity> {
    ...
    @Select("SELECT * FROM lab_config WHERE space_id = #{spaceId} ORDER BY revision DESC FOR UPDATE")
    List<LabConfigEntity> selectBySpaceIdForUpdate(@Param("spaceId") String spaceId);
}
