...
2. 页面选择设备联调参数集；它是 Action 唯一的运行参数来源。
3. `POST /api/action-executions/preview` 解析 `$parameters.*`，返回最终 `MainAction`、`resolvedSteps` 和 `packageHash`。
4. 正式执行必须回传该 `packageHash`；预览后任何 Action 或参数变化都会导致后端拒绝执行。
5. 后端先持久化定义、参数与命令快照，再通过 TCP 下发整包。
